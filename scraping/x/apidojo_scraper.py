...
class ApiDojoTwitterScraper(Scraper):
    ...
    def _best_effort_parse_tweets(self, tweets: list[Tweet], check_engagement: bool = True) -> Tuple[List[XContent], List[bool], int]:
        ...
        for tweet in tweets:
            try:
                ...
                if check_engagement:
                    if tweet.user and utils.is_spam_account2(tweet.user):
                        bt.logging.debug(f"Filtered spam account: {tweet.user.username}")
                        continue
